...
class DeterministicRunner(DynamicModel):
    ...
    def __init__(self, modelTime, model_setup):
        ...
        # read the additional mask
        logger.debug(f"Reading global wetland mask from {self.model_setup['global_wetland_mask_file']}")
#        self.global_wetland_mask = vos.readPCRmapClone(v=self.model_setup["global_wetland_mask_file"], 
#                                                       cloneMapFileName=self.clone, 
#                                                       tmpDir=self.tmp_folder, 
#                                                       absolutePath=None, 
#                                                       isLddMap=False, 
...
#                                                       isNomMap=False)
        ...
        self.global_wetland_mask = vos.readPCRmapClone(v= global_wetland_mask_file, 
                                                       cloneMapFileName=self.clone, 
                                                       tmpDir=self.tmp_folder)
        self.global_wetland_mask = pcr.ifthen(pcr.defined(self.global_wetland_mask), pcr.boolean(1.0))
        
        # combine the additional mask with the landmask
        self.combined_mask = pcr.ifthen(self.landmask & self.global_wetland_mask, pcr.boolean(1.0))

        # initiate a netcdf writer
        self.netcdf_report = netcdf_writer.PCR2netCDF(self.model_setup["clone_file"])
        self.netcdf_report.createNetCDF(self.model_setup["saturated_area_fraction_output_file"], "satAreaFrac", "-")

    def initial(self):
        
        # read soil parameters from the netCDF file:
        soilParameters = ['resVolWC1', 'resVolWC2', 'satVolWC1', 'satVolWC2']
        ...
        self.thickUpp   = 0.3
        self.thickLow   = 1.2
        self.storCapUpp = self.thickUpp * (self.satVolWC1 - self.resVolWC1)
        self.storCapLow = self.thickLow * (self.satVolWC2 - self.resVolWC2)
        ...
        self.naturalFracVegCover = {"forest": self.fraction_forest, "grassland": self.fraction_grassland}
        ...
        self.irrTypeFracOverIrr = {"irrPaddy": vos.getValDivZero(fraction_paddy_over_irrigated_land, total_irrigated_land_fraction_before_correction),
                                   "irrNonPaddy": vos.getValDivZero(fraction_non_paddy_over_irrigated_land, total_irrigated_land_fraction_before_correction)}
    ...
    def dynamic(self):
        ...
        # at the beginning of every year, read/set land cover parameters (related to the Arno Scheme) based on the extents of irrigation areas and natural land cover areas
        if self.modelTime.isFirstDayOfMonth() or self.modelTime.isFirstTimestep():
            ...
            if self.dynamicIrrigationAreaFile.endswith(('.nc4', '.nc')):
                ...
                self.irrigationArea = 10000. * pcr.cover(vos.netcdf2PCRobjClone(self.dynamicIrrigationAreaFile, 
                                                                                'irrigationArea', 
                                                                                fulldateInString, 
                                                                                useDoy='yearly', 
            ...
            # calculate the aggregate land cover parameters
        ...
        if self.modelTime.isLastDayOfMonth():
            ...
            saturated_area_fraction = 1.00 - ((self.rootZoneWaterStorageCap - monthly_total_soil_storage) / 
                                              (self.rootZoneWaterStorageCap - self.rootZoneWaterStorageMin)) ** (self.arnoBeta / (self.arnoBeta + 1))
            # - set the minimum to zero
            saturated_area_fraction = pcr.max(0.0, saturated_area_fraction)

            # apply the additional mask
            saturated_area_fraction = pcr.ifthen(self.combined_mask, saturated_area_fraction)
...
def main():
    ...
    model_setup["output_dir"] = "/scratch-shared/otoo0001/test_sat_area_frac/test_wetlands/"
    ...
    # path to the additional mask file
    model_setup["global_wetland_mask_file"] = "/scratch-shared/otoo0001/GLOBGDES/Paper2_mapping_wetlands/Wetland_data(validated)/resampled_wetland_boolean_5arcmin.map"
